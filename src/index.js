...
export default function createIndexedDB(name = 'indexedDB') {
   ...
   * @param {string} store 可选. 不传会将undefined转为字符串'undefined'
   * @returns {Promise}
   */
   ...
   * @param {string} store  可选. 要创建的objectStore的名字, 不传会将undefined转为字符串'undefined'
   * @param {string} index  可选. 需要创建objectStore索引时传入,key为字段名,value为boolean表示是否允许重复
   * @param {string} key   可选. 主键名, 对应每条数据必须为包含keyPath属性的对象; 不传则使用主键自增(默认从1开始, 如果之前有number类型的主键, 会对最大一个number类型主键取整数, 然后加1作为自增后的主键)
   * @param {boolean} replace  可选. 如果表存在是否先删除再创建, 默认不删除不创建
   * @returns {Promise}
   */
  ...
  /**
   * 删除objectStore
   * @param {string} store 可选. 删除的objectStore名, 不传会将undefined转为字符串'undefined'
   * @returns {Promise}
   */
   ...
   * @param {string} store 可选. 需要查询数据的objectStore名, 不传会将undefined转为字符串'undefined'
   * @param {string} key 必选. 主键
   * @returns {Promise}
   */
   ...
   * @param {string} store   可选. 需要查询数据的objectStore名, 不传会将undefined转为字符串'undefined'
   * @param {string} index 可选. 索引名, 如果不传索引, start和end为主键范围
   ...
   * @param {string} direction 可选. 默认next. 光标的遍历方向, 值为以下4个: 'next'(下一个),'nextunique'(下一个不包括重复值),'prev'(上一个),'prevunique'(上一个不包括重复值)
   * @param {function} filter 可选. 过滤数据方法
   * @returns {Promise}
   */
   ...
   * @param start  可选. 索引的起始值, 查询表中所有数据start和end都不传即可; 只查询大于start的数据, end不传即可
   * @param end  可选. 索引结束值, 只查单个索引, 传入跟start相同的值即可; 查询所有小于end的数据, start不传即可
   * @param {number} page 可选. 默认1. 页码, Number
   * @param {number} pageSize 可选. 默认10. 每页有多少条数据, Number
   * @param {string} direction 可选. 光标的遍历方向, 值为以下4个: 'next'(下一个),'nextunique'(下一个不包括重复值),'prev'(上一个),'prevunique'(上一个不包括重复值)
   * @param {function} filter 可选. 过滤数据方法
   * @returns {Promise}
   */
   ...
   * @param {string} store  可选. 需要添加/修改数据的objectStore名, 不传会将undefined转为字符串'undefined'
   ...
   * @param {string} store  可选. 需要删除数据的objectStore名, 不传会将undefined转为字符串'undefined'
   * @param {string} index 可选. 索引名, 如果不传索引, start和end为主键范围
